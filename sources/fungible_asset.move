module aptos_asset::fungible_asset {
    ...
    public entry fun transfer(admin: &signer, from: address, to: address, amount: u64) acquires ManagedFungibleAsset {
        let asset = get_metadata(); //fetching the metadata object associated with the fungible asset.
        let transfer_ref = &authorized_borrow_refs(admin, asset).transfer_ref;
        let from_wallet = primary_fungible_store::primary_store(from, asset); //retrieve the sender's wallet.
        let to_wallet = primary_fungible_store::ensure_primary_store_exists(to, asset);//Ensures the recipient's wallet exists. If not, this line creates one.
     ...
     public entry fun burn(admin: &signer, from: address, amount: u64) acquires ManagedFungibleAsset {
        ...
        let from_wallet = primary_fungible_store::primary_store(from, asset);//Retrieves the wallet of the address from which tokens are to be burned
